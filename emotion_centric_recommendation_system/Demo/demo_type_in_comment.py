...
# Load the music video transcripts
youtube_mv_transcripts = pd.read_csv('youtube_mv_predicted_emotion.csv')

# Ask for text or text file 
user_comment = st.text_input('Enter comment here') 

# Create a new dataframe 'youtube_comments' that replicates 'text'
youtube_comments = pd.DataFrame(data={'text': [user_comment], 'comment_origin_video_id': ['']})

# Tokenize and pad sequences
youtube_comments['text_to_model'] = youtube_comments['text'].apply(lambda x: tokenizer.texts_to_sequences([x]))
...
selected_row = youtube_comments.iloc[0]
st.header("User Comment")
st.write(f"Comment: {selected_row['text']}")
...
def recommend_music_videos(user_comment, user_emotion, youtube_mv_transcripts, youtube_comments):
    ...
    for i, video_id in enumerate(video_ids):
        ...
        if user_emotion in negative_emotions:
            ...
            st.markdown(f"**Cosine similarity score:** {recommended_videos.iloc[i]['cosine_similarity']}")
        else:
...
if st.button("Generate Music Video Recommendations"):
    st.spinner()
    recommend_music_videos_by_index(0)